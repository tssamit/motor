...
switch ($action) {
    case 'UsercPageSet':
        ...
        if ($data->num_rows > 0) {
            ...
            header("Location: ../add-network-manager");
        } else {
            ...
            if ($user_pwd == $CPassword) {
                if (!empty($_POST['checkbox_id'])) {
                    ...
                    //  exit();
                    $db->query("INSERT INTO `admin` (`a_id`, `a_code`, `a_usertype`, `a_username`, `a_password`, `a_vpwd`, `a_name`, `a_email`, `a_phone`, `a_page`, `a_icon`, `a_aadhar_no`) VALUES (NULL, '$codeno', '$a_usertype', '$user_id', '$a_password', '$CPassword', '$user_name', '$a_email', '$a_phone',  '$path', '$path' , '$a_aadhar_no')");
