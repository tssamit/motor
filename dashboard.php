    ...
    <body data-theme="<?= $datatheme; ?>" data-layout="fluid" data-sidebar-position="left" data-sidebar-layout="default">
        <div class="wrapper">
            ...
            <div class="main">
                ...
                <?php
                $sqltype = $db->query("SELECT * FROM `vw_master_user` WHERE a_usertype = '" . $_SESSION['LOGAUTHType'] . "'");
                $querytype = $sqltype->fetch_object();
                ?>
                <main class="content">
                    <div class="container-fluid p-0">
                        ...
                        <?php
                        if ($_SESSION['LOGAUTHType'] == '3') {
                            if ($querytype->a_upgrade == '1') {
                                ?>
                                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                    <a href="business-detail" class="btn btn-info" type="button">Business Detail</a>
                                </div>
                            <?php } ?>
                            <?php
                            if ($querytype->a_upgrade == '') {
                                ?>
                                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                    <a href="pages/action-bacceptance.php?submit=sreqst&id=<?= $_SESSION['a_id']; ?>" class="btn btn-primary" type="button">Business Upgrade Request</a>
                                </div>
                            <?php } ?>
                        <?php } ?>

                        <?php
                        if ($_SESSION['LOGAUTHType'] == '7') {
                            ?>
                            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                <a href="scanqr.php" class="btn btn-primary btn-lg" type="button">Scan QR Code</a>
                            </div>
                        ...
                        <div class="row mb-2 mb-xl-3">
                            <div class="col-auto d-none d-sm-block">
                                <h3><strong>Welcome to</strong> Dashboard</h3>
                            </div>
                            ...
                            <div class="row">
                                <?php
                                if (!empty($querytype->a_icon_permission)) {
                                    foreach (explode(',', $querytype->a_icon_permission) as $nd) {
                                        $resultsnavd2 = $db->query("SELECT page_title,menu_cust_name,path,desktop_icon FROM vw_menu WHERE menu_status ='Active' AND menu_code = '$nd'");
                                        $datanav2 = $resultsnavd2->fetch_object();
                                        if (!empty($datanav2->path)) {
                                            ?>
                                            <div class="col-md-2 col-sm-6 col-xs-6">
                                                <a href="<?= $datanav2->path; ?>" title="<?= $datanav2->page_title; ?>">
                                                    <center>
                                                        <i class="fa <?= $datanav2->desktop_icon; ?> fa-5x text-dark" aria-hidden="true"></i> <br>
                                                        <span class="btn btn-link text-dark" style="margin-top: 5px;"><?= $datanav2->menu_cust_name; ?></span>
                                                    </center>
                                                </a>
                                            </div>
